...
app.get('/r/:slug', async (req, res) => {
  ...
  if (!isValidSlug(slug)) {
    res.status(400).type('text/html').send(renderReceiptPage(null, slug, 'invalid'));
    return;
  }
  const record = await loadSubmission(slug).catch(() => null);
  if (!record) {
    res.status(404).type('text/html').send(renderReceiptPage(null, slug, 'missing'));
    return;
  }
  res.type('text/html').send(renderReceiptPage(record, slug, 'ok'));
});
...
app.get('/r/:slug/admin/:token', async (req, res) => {
  ...
  if (!isValidSlug(slug) || !isValidAdminToken(token)) {
    res.status(400).type('text/html').send(renderShellHtml('Reply', '<p>Invalid admin link.</p>'));
    return;
  }
  const record = await loadSubmission(slug).catch(() => null);
  if (!record || record.adminToken !== token) {
    res.status(404).type('text/html').send(renderShellHtml('Reply', '<p>Admin link not found or no longer valid.</p>'));
    return;
  }
  const flash = req.query.ok === '1' ? 'sent' : null;
  res.type('text/html').send(renderAdminPage(record, slug, token, flash, null));
});
...
app.post('/r/:slug/admin/:token', async (req, res) => {
  ...
  if (!reply) {
    res.status(400).type('text/html').send(renderAdminPage(record, slug, token, null, 'Reply cannot be empty.'));
    return;
  }
  if (reply.length > 5000) {
    res.status(400).type('text/html').send(renderAdminPage(record, slug, token, null, 'Reply too long (max 5000 characters).'));
    return;
  }
  try {
    ...
    res.status(500).type('text/html').send(renderAdminPage(record, slug, token, null, 'Could not save the reply. Try again.'));
    return;
  }
  res.redirect(303, `/r/${slug}/admin/${token}?ok=1`);
});
...
function renderShellHtml(title, bodyHtml) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex"><title>${escapeHtml(title)}</title><style>${RECEIPT_CSS}</style></head><body>${bodyHtml}<footer>voice-to-us · <a href="/widget-demo" style="color:inherit">demo</a></footer></body></html>`;
}
...
function renderReceiptPage(record, slug, state) {
  ...
  return renderShellHtml(title, main);
}

function renderAdminPage(record, slug, token, flash, errorMsg) {
  const rows = metadataRowsHtml(record);
